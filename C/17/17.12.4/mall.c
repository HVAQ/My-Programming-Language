...
bool newcustomer (double x);  // 有顾客到来吗
...
int main (void)
{
	...
	long line_wait = 0;  // 队列累积等待时间

	InitializeQueue (&line);
	srand (time(0));  // 初始化种子
	puts ("Case study: Sigmund Lander's Advice Booth");
	puts ("Enter the number of simulation hours: ");
	scanf ("%d", &hours);  // 获取模拟小时数
	cyclelimit = MIN_PER_HR * hours;  // 以分计
	puts ("Enter the average number of customers per hour: ");
	scanf ("%d", &perhour);  // 每小时顾客的平均数
	min_per_cust = MIN_PER_HR / perhour;  // 顾客到达的平均时间间隔
	...
	for (cycle = 0; cycle < cyclelimit; cycle++)
	{
		...
		{
			if (QueueIsFull (&line, linenum))
				turnaways++;
			else
			{
				customers++;
				temp = customertime (cycle);
				EnQueue (temp, &line, linenum);
			}
		}
